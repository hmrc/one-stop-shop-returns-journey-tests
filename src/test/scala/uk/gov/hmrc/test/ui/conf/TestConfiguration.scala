...
object TestConfiguration {
  ...
  def url(service: String): String = {
    val host = env match {
      ...
      case _       => s"${environmentProtocol(service)}://${envConfig.getString(s"services.host")}"
    }
    s"$host${serviceRoute(service)}"
  }

  def environmentProtocol(serviceName: String): String = envConfig.getString(s"$serviceName.protocol")
