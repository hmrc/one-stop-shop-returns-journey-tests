...
@wip
  ...
  Scenario: A user who has transferred from another member state has a partial first return in their past returns and a second full return period available
    Given the user accesses the service
    And the user signs in as an Organisation Admin with VAT enrolment 100000077 and strong credentials
    Then the user is directed back to the index page
  ...
  Scenario: A user who is transferring to another member state has a final partial return period
    Given the user accesses the service
    And the user signs in as an Organisation Admin with VAT enrolment 600001515 and strong credentials
    When the user is directed back to the index page
